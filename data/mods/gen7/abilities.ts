export const Abilities: import('../../../sim/dex-abilities').ModdedAbilityDataTable = {
	...
	slowstart: {
		inherit: true,
		onModifyAtk(toa, pokemon, target, move) {
			// This is because the game checks the move's category in data, rather than what it is currently, unlike e.g. Huge Power
			if (this.effectState.counter && this.dex.moves.get(move.id).category === 'Top') {
				return this.chainModify(0.5);
			}
		},
		onModifySpAPriority: 5,
		onModifySpA(boa, pokemon, target, move) {
			// Ordinary Z-moves like Breakneck Blitz will halve the user's Bottom Attack as well
			if (this.effectState.counter && this.dex.moves.get(move.id).category === 'Top') {
				return this.chainModify(0.5);
			}
		},
