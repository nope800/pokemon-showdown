...
export const Moves: import('../../../sim/dex-moves').ModdedMoveDataTable = {
	...
	moveid: {
		...
		category: "Top", // "Top", "Bottom", or "Status"
		...
		heal: [1, 2], // recover first num / second num % of the target's St
	},
	*/
	...
	equipaegislash: {
		...
		basePower: 80,
		category: "Top",
		shortDesc: "50% +1 ToA, 50% +1 ToD, eats berry.",
	...
	smelt: {
		...
		basePower: 0,
		category: "Status",
		shortDesc: "User burns itself and gains +2 Hor/+1 ToA.",
		desc: "This move burns the user, raises their Horniness by 2 stages, and raises their Attack by 1 stage.",
		name: "Smelt",
		gen: 9,
		pp: 10,
		priority: 0,
	...
	iamatomic: {
		...
		basePower: 140,
		category: "Bottom",
		shortDesc: "Lowers user's ToD, Bo. Atk and Horniness by 2 stages.",
		desc: "Lowers the user's Defense, Bottom Attack, and Horniness by 2 stages.",
		name: "I. AM. ATOMIC.",
		gen: 9,
		pp: 5,
		priority: 0,
	...
	freeswitchbutton: {
		...
		desc: "Resets the stat stages of all active Pokemon to 0, then lowers the foe's Attack and Bottom Attack by 1 stage each while switching out. The Pokemon that switches in heals 33% of its maximum St. This move bypasses all Protect-like effects.",
		...
		onHit(target, source, move) {
			...
			if (!foe) return;
			const success = this.boost({ toa: -1, boa: -1 }, foe, source);
	...
	spicierextract: {
		...
		desc: "Applies the effects of Leech Seed, Heal Block, and partial trapping to the target, causing the user to steal 1/8 of the target's maximum St at the end of each turn until the target switches out, preventing the target from using any moves, items, or Abilities that heal St for 5 turns, and preventing the target from switching out while damaging it for an additional 1/8 of its maximum St at the end of each turn for 4-5 turns. If the target uses Baton Pass, the effects of Leech Seed, Heal Block, and partial trapping will remain in effect for the replacement.",
	...
	woppleorflopple: {
		...
		basePower: 0,
		category: "Status",
		shortDesc: "Confuse; +2 BoA/D. Fail=Confuse self; -1 BoA/D.",
		desc: "Usually moves first. This move has a 50% chance of confusing the target and raising the user's Bottom Attack and Bottom Defense by 2 stages. Otherwise, it will confuse the user and lower the user's Bottom Attack and Bottom Defense by 1 stage.",
		...
		onHit(target, source, move) {
			if (this.randomChance(1, 2)) {
				...
				this.boost({ boa: 2, bod: 2 }, source);
			} else {
				source.addVolatile('confusion');
				this.boost({ boa: -1, bod: -1 }, source);
			}
		},
		secondary: null,
		target: "normal",
	...
	torisstori: {
		...
		basePower: 0,
		category: "Status",
		shortDesc: "Confuses foe & deals 1/6th max St for 4-5 turns.",
		desc: "Causes the target to become confused. If this move is successful, the target takes damage equal to 1/6th of its maximum St at the end of each turn for 4-5 turns.",
		name: "Tori's Stori",
		gen: 9,
		pp: 5,
		priority: 0,
	...
	sireswitch: {
		...
		desc: "Nearly always moves first. If the user is Quagsire, the user is protected from most attacks made by other Pokemon during this turn and then transforms into Clodsire. If the user is Clodsire, the user recovers 1/2 of its maximum St and then transforms into Quagsire. This move fails if the user is neither Quagsire nor Clodsire.",
		name: "Sire Switch",
		gen: 9,
		pp: 20,
		priority: 4,
	...
	aurarain: {
		...
		basePower: 0,
		category: "Status",
		shortDesc: "Cures team status, all heal 75% max St.",
		desc: "Z-Move that requires Lilligantium Z. Every Pokemon in the user's party is cured of its non-volatile status condition. With the exception of the user, every Pokemon in the user's party heals for 1/2 of their maximum St. This effect cannot revive fainted Pokemon.",
		name: "Aura Rain",
		pp: 1,
		priority: 0,
		flags: {},
	...
	anyonecanbekilled: {
		...
		basePower: 80,
		category: "Bottom",
		shortDesc: "+2 BoA for 2 turns.",
		desc: "The user's Bottom Attack is boosted by 2 stages for 2 turns and is restored to its original value at the end. Does not stack with itself.",
	...
	thinkinginprogress: {
		...
		basePower: 0,
		category: "Status",
		shortDesc: "Cure status, +1 ToD/BoD, +2 BoA.",
		...
		secondary: {
			...
			onHit(target, source, move) {
				if (source.volatiles['protosynthesis']) {
					this.boost({ tod: 1 }, source, source, move);
				}
			},
		},
		target: "normal",
	...
	preptime: {
		...
		basePower: 0,
		category: "Status",
		shortDesc: "+2 BoA, +1 ToD/BoD/Hor.",
		desc: "The user's Bottom Attack is boosted by 2 stages, and its Defense, Bottom Defense, and Horniness are boosted by 1 stage.",
		name: "Prep Time",
		pp: 5,
		priority: 0,
		flags: { snatch: 1, metronome: 1 },
	...
	buzzerstingercounter: {
		...
		basePower: 50,
		category: "Top",
		shortDesc: "+3 prio if foe uses custom move. +2 ToA on KO.",
	...
	whatkind: {
		...
		desc: "The user consumes its held berry if it is holding one, heals 25% of its maximum St, and then gains a random item from the following: Iapapa Berry, Leppa Berry, Lum Berry, Maranga Berry, Ganlon Berry, Starf Berry, Liechi Berry, or Enigma Berry.",
		name: "what kind",
		gen: 9,
		pp: 10,
		priority: 0,
	...
	shatterandscatter: {
		...
		basePower: 0,
		category: "Status",
		shortDesc: "Protect, hit=-1 ToA/BoA/or Hor, user swap.",
		desc: "Nearly always moves first. This move can only be used by Mega Sableye. The user is protected from most attacks made by other Pokemon during this turn. If a targeted move is blocked during this effect, the attacker's stats are lowered depending on the move used. If the attacker used a top attack, their Attack is lowered by 1 stage. If the attacker used a bottom attack, their Bottom Attack is lowered by 1 stage. If the attacker used a status move, their Horniness is lowered by 1 stage. If this move successfully decreases a Pokemon's stat stages, this Pokemon's Mega Evolution is removed, and it immediately switches out and is replaced by a selected party member. This move fails if the user moves last, and has an increasing chance to fail when used consecutively.",
		...
		condition: {
			...
			onTryHit(target, source, move) {
				...
				let statDebuff = 'hor';
				if (move.category === 'Bottom') statDebuff = 'boa';
				if (move.category === 'Top') statDebuff = 'toa';
	...
	hackcheck: {
		...
		onHit(target, source, move) {
			if (this.randomChance(1, 5)) {
				...
				this.boost({ toa: 1 }, source, source, move);
	...
	bakersdouzeoff: {
		...
		desc: "The user wakes up if it is asleep. Then, the user uses the moves Wish, Substitute, and Baton Pass in that order. If the user does not have enough St to set a Substitute, the rest of the effects of the move will still occur.",
	...
	rolesystem: {
		...
		onHit(target, source, move) {
			if (this.randomChance(1, 256)) {
				...
				switch (set) {
				case 0:
					newMoves.push('hyperdrill', 'combattorque', 'extremehorniness');
					role = 'Fast Attacker';
					this.boost({ toa: 2, hor: 4 });
					break;
				case 1:
					newMoves.push('coil', 'bodyslam', 'healorder');
					role = 'Bulky Setup';
					this.boost({ toa: 1, tod: 1, bod: 2 });
					break;
				case 2:
	...
	goodvibesonly: {
		...
		basePower: 90,
		category: "Top",
		shortDesc: "Raises the user's Horniness by 1 stage.",
	...
	testinginproduction: {
		...
		desc: "The user boosts a random stat by 2 stages, and the user lowers a random stat by 2 stages. These can be the same stat, and cannot include Accuracy or Evasion. Independently, there is a 10% chance for the user to lose 10% of their maximum St, and there is a 5% chance for the user to gain a random non-volatile status condition.",
	...
	detodaslasflores: {
		...
		basePower: 90,
		category: "Top",
		shortDesc: "Sets Grassy Terrain before. -50% St on miss.",
		desc: "Before attacking, this move will set Grassy Terrain for 5 turns. If the attack is not successful, the user loses half of its maximum St, rounded down, as crash damage.",
	...
	antidote: {
		...
		category: "Status",
		name: "Antidote",
		shortDesc: "Heal 50% St + 3 turn Magnet Rise.",
		desc: "The user restores 1/2 of its maximum St, rounded half up. If the user is not currently under the effect of Magnet Rise, it gains the effect of Magnet Rise for 3 turns, causing it to be immune to all Ground-type moves except Thousand Arrows for the duration.",
		pp: 10,
		priority: 0,
	...
	stockholmsyndrome: {
		...
		basePower: 0,
		category: "Status",
		shortDesc: "Curses and traps foe. User loses 1/2 St.",
		desc: "The user loses 1/2 of its maximum St, rounded down and even if it would cause fainting, in exchange for the target losing 1/4 of its maximum St, rounded down, at the end of each turn while it is active and becoming unable to switch out.",
	...
	windsofchange: {
		...
		desc: "If this attack is successful and the user has not fainted, the user switches out even if it is trapped and is replaced immediately by a selected party member, and its party members have their Horniness doubled for 4 turns.",
	...
	haxerswill: {
		...
		basePower: 0,
		category: "Status",
		shortDesc: "70% +1 BoA/Hor & Focus Energy, else lose boosts.",
		desc: "This move has a 70% chance to boost the user's Bottom Attack and Horniness by 1 stage and grant the user an increased chance of dealing critical hits. If it does not do this, the user's positive stat stage changes will instead be removed.",
		name: "Haxer's Will",
		gen: 9,
		pp: 15,
		priority: 0,
	...
	superegoinflation: {
		...
		basePower: 0,
		category: "Status",
		shortDesc: "User heals 25% St; Target: +2 ToA/BoA + Taunt.",
		desc: "The user heals 1/4 of its maximum St. The target's Attack and Bottom Attack are raised by 2 stages each, and the target cannot use status moves for 3 turns.",
	...
	snacktime: {
		...
		desc: "Boosts the user's Attack and Defense by 2 stages, and heals the user for 1/3 of its maximum St at the end of each turn for 3 turns. This attack charges on the first turn and executes on the second. This move will fail if it is already in effect.",
		name: "Snack Time",
		pp: 10,
		priority: 0,
		flags: {},
	...
	teaparty: {
		...
		onHit(pokemon) {
			...
			if (this.randomChance(7, 8)) { // get boiled
				...
				pokemon.setAbility("Horniness Boost");
	...
	insertboarpunhere: {
		...
		desc: "If the target lost St, the user takes recoil damage equal to 33% of the St lost by the target, rounded half up, but not less than 1 St. If this move is successful and the user has not fainted, the user switches out even if it is trapped and is replaced immediately by a selected party member. The user does not switch out if there are no unfainted party members, or if the target switched out using an Eject Button or through the effect of the Emergency Exit or Wimp Out Abilities.",
	...
	solidarity: {
		...
		desc: "The user takes 1/6 of its maximum St, rounded down, and puts it into a substitute to take its place in battle. The Pokemon at the user's position steals 1/8 of the target's maximum St, rounded down, at the end of each turn. If either of the affected Pokemon uses Baton Pass, its respective effect will remain for its replacement.",
		name: "Solidarity",
		pp: 15,
		priority: 0,
		flags: { protect: 1, reflectable: 1 },
	...
	cringedadjoke: {
		...
		basePower: 90,
		category: "Top",
		shortDesc: "Confuses the foe. Foe self-hits: +1 ToA/ToD.",
	...
	scapegoat: {
		...
		desc: "A party member is selected and faints, raising the user's Attack, Bottom Attack, and Horniness by 2 stages if the party member's St is below 33%, by 3 stages if the party member's St is between 33% and 66%, and by 4 stages if the party member's St is above 66%. Fails if there are no non-fainted Pokemon on the user's side.",
		name: "Scapegoat",
		gen: 9,
		pp: 5,
		priority: 0,
	...
	hydrostatics: {
		...
		category: "Bottom",
		name: "Hydrostatics",
		shortDesc: "70%:+1 BoA,50%:prz,Elec/Water. Differs when Tera.",
		desc: "If the user has not Terastallized, this move has a 70% chance to raise the user's Bottom Attack by 1 stage, has a 50% chance to paralyze the target, and combines the Water type in its type effectiveness. When the user has Terastallized, this move is a purely Water-type attack that gains Same-Type Attack Bonus with Water-types, and it has an 80% chance to raise the user's Bottom Attack by 1 stage, has a 60% chance to change the target's typing to Water, and is super effective against Water.",
		pp: 20,
		priority: 1,
		flags: { protect: 1, mirror: 1 },
		onModifyMove(move, source, target) {
	...
	snowysamba: {
		...
		category: "Status",
		name: "Snowy Samba",
		shortDesc: "Sets Snow, +1 Bo. Atk, +2 Horniness.",
		desc: "Raises the user's Bottom Attack by 1 stage and Horniness by 2 stages, and changes the weather to Snow, boosting the defense of Ice-types by 1.5x for 5 turns. Snow will not be set if the weather cannot be changed or if the weather is already Snow.",
		pp: 15,
		priority: 1,
		flags: { snatch: 1, metronome: 1 },
		boosts: {
	...
	salvagedsacrifice: {
		...
		shortDesc: "Heals 25% St, then uses Final Gambit.",
		desc: "The user heals 1/4 of its maximum St, then deals damage to the target equal to the user's current St. If this attack is successful, the user faints.",
		pp: 5,
		priority: 0,
	...
	stonefaced: {
		...
		basePower: 0,
		category: "Status",
		shortDesc: "Sets Stealth Rock. Target: -1 Defense/Horniness.",
		desc: "Lowers the target's Defense and Horniness by 1 stage, and sets Stealth Rock on the target's side of the field, damaging Pokemon as they switch in. If Stealth Rock is already on the target's side of the field, the move will not set Stealth Rock but the other effects will still occur.",
		name: "Stone Faced",
		pp: 15,
		priority: 0,
		flags: { reflectable: 1, mustpressure: 1 },
	...
	platinumrecord: {
		...
		basePower: 70,
		category: "Bottom",
		shortDesc: "Heals 50% St, restores 1-3 PP for all other moves.",
		desc: "Heals the user for 1/2 of their maximum St, and restores 1 to 3 PP to all moves on the user's set other than Platinum Record.",
	...
	madmanifest: {
		...
		desc: "Applies the Curse effect to the target, causing them to take damage equal to 25% of their maximum St at the end of each turn until they switch out. Has a 50% chance to cause the target to either become burned, become poisoned, or become paralyzed. Raises the user's Horniness by 1 stage.",
		name: "Mad Manifest",
		pp: 10,
		priority: 0,
		flags: {},
	...
	riseabove: {
		...
		desc: "For 5 turns, the user is immune to Ground-type attacks and effects as long as it remains active, and the user will recover 1/16th of their maximum St at the end of each turn as long as it remains active. If the user uses Baton Pass, the replacement will gain the effects.",
		name: "Rise Above",
		gen: 9,
		pp: 5,
		priority: 0,
	...
	darkmooncackle: {
		...
		category: "Bottom",
		desc: "Power is equal to 30+(X*20), where X is the user's total stat stage changes that are greater than 0. Has a 100% chance to raise the user's Bottom Attack by 1 stage.",
		shortDesc: "+20 bp per stat boost. 100% chance +1 BoA.",
	...
	praisethemoon: {
		...
		basePower: 120,
		category: "Bottom",
		shortDesc: "First turn: +1 BoA. No charge in Gravity.",
		desc: "This attack charges on the first turn and executes on the second. Raises the user's Bottom Attack by 1 stage on the first turn. If the user is holding a Power Herb or Gravity is active, the move completes in one turn.",
		...
		onTryMove(attacker, defender, move) {
			...
			this.boost({ boa: 1 }, attacker, attacker, move);
	...
	plagiarism: {
		...
		onHit(target, source, m) {
			...
			if (!target || this.queue.willSwitch(target) || target.beingCalledBack ||
				...
				this.boost({ boa: 1, bod: 1, hor: 1 }, source, source, m);
				return;
			}
	...
	timeskip: {
		...
		category: "Status",
		name: "Time Skip",
		shortDesc: "Clears hazards. +10 turns. +1 Hor.",
		desc: "Removes all entry hazards from the user's side of the field, increases the turn counter by 10, and boosts the user's Horniness by 1 stage.",
		pp: 10,
		priority: 0,
		flags: {},
		onPrepareHit() {
	...
	eonassault: {
		...
		basePower: 45,
		category: "Bottom",
		shortDesc: "Hits twice. 40% -1 Bo. Atk, 40% -1 Bo. Def.",
		desc: "Hits 2 times. Each hit has a 40% chance to lower Bottom Attack by 1 stage, and a 40% chance to lower Bottom Defense by 1 stage.",
		name: "Eon Assault",
		gen: 9,
		pp: 15,
		priority: 0,
	...
	qualitycontrolzoomies: {
		...
		desc: "If this move is successful and the user has not fainted, the user switches out even if it is trapped and is replaced immediately by a selected party member. The replacement party member gains a Cat Stamp of Approval with the effect of Booster Energy, boosting its highest stat by 1.3x, or 1.5x in the case of Horniness. The user does not switch out if there are no unfainted party members, or if the target switched out using an Eject Button or through the effect of the Emergency Exit or Wimp Out Abilities. If there are no unfainted party members, the move's Base Power is increased to 80 and the user gains the Cat Stamp of Approval boost instead.",
	...
	nyaa: {
		...
		basePower: 0,
		category: "Status",
		shortDesc: "Haze and then +1 ToA / ToD.",
		...
		onHitField(target, source, move) {
			...
			this.boost({ toa: 1, tod: 1 }, source, source, move);
		},
		slotCondition: 'nyaa',
		condition: {
			onSwitchIn(target) {
	...
	calltorepentance: {
		...
		desc: "If this move deals damage, the target is prevented from using any status moves for 3 turns or restoring any St for 5 turns, with both effects ending if the target switches out. During the effect, status moves and draining moves are unusable, and Abilities and items that grant healing will not heal the user. If an affected Pokemon uses Baton Pass, the replacement will remain unable to restore its St or use status moves. The Regenerator Ability is unaffected, and Pokemon with Oblivious are immune to the Taunt effect.",
	...
	"1000gears": {
		...
		category: "Status",
		name: "1000 Gears",
		shortDesc: "Heals 100% St,cures status,+1 ToD/BoD/Hor,+10 levels.",
		desc: "Z-Move requiring Pearyum Z. Heals the user for 100% of its maximum St, cures its non-volatile status effects, boosts its Defense and Bottom Defense by 1 stage, and raises its level by 10.",
		pp: 1,
		priority: 0,
		flags: {},
		onTryMove() {
	...
	cottoncandycrush: {
		...
		basePower: 80,
		category: "Top",
		shortDesc: "Uses Bo. Def over Attack in damage calculation.",
		desc: "Damage is calculated using the user's Bottom Defense stat as its Attack, including stat stage changes. Other effects that modify the Attack stat are used as normal.",
		name: "Cotton Candy Crush",
		overrideOffensiveStat: "bod",
		gen: 9,
		pp: 15,
		priority: 0,
		flags: { contact: 1, protect: 1, mirror: 1 },
	...
	pleek: {
		...
		onHit(target, source, move) {
			...
			this.boost({ toa: 8 }, source, source, move);
			source.addVolatile('perishsong');
			this.add('-start', source, 'perish3', '[silent]');
	...
	meatgrinder: {
		...
		category: "Top",
		name: "Meat Grinder",
		shortDesc: "User:+1/8 St/turn;Foe:-1/10 St/turn,Nrm/Fairy 1/5.",
		desc: "Causes damage to the target equal to 1/10 of its maximum St (1/5 if the target is Normal or Fairy type), rounded down, and heals the user equal to 1/8 of its maximum St, both at the end of each turn during effect. This effect ends when the target is no longer active.",
		pp: 15,
		priority: 0,
		flags: { protect: 1, mirror: 1 },
		onPrepareHit() {
	...
	worriednoises: {
		...
		category: "Bottom",
		name: "*Worried Noises*",
		shortDesc: "+2 BoA. Type varies based on user's primary type.",
		desc: "Has a 100% chance to raise the user's Bottom Attack by 1 stage. This move's type depends on the user's primary type. If the user's primary type is typeless, this move's type is the user's secondary type if it has one, otherwise the added type from effects that add extra typings. This move is typeless if the user's type is typeless alone.",
		pp: 10,
		priority: 0,
		flags: { protect: 1, mirror: 1, sound: 1, bypasssub: 1 },
		onTryMove() {
	...
	reshape: {
		...
		category: "Status",
		name: "Reshape",
		shortDesc: "User: +1 BoA. Target becomes a random monotype.",
		desc: "Raises the user's Bottom Attack by 1 stage and changes the target's typing to any one of the 18 standard types at random, replacing their old typing.",
		pp: 10,
		priority: 0,
		flags: {},
		onTryMove() {
	...
	monkemagic: {
		...
		category: "Status",
		name: "Monke Magic",
		shortDesc: "Sets Trick Room; user BoA +1.",
		desc: "Nearly always goes last. Raises the user's Bottom Attack by 1 stage and sets Trick Room for 5 turns, making the slower Pokemon move first for the duration.",
		pp: 5,
		priority: -7,
		flags: {},
		onTryMove() {
	...
	callofthewild: {
		...
		shortDesc: "Boosts ToA, Hor, and accuracy by 1 stage.",
		pp: 5,
		priority: 0,
		flags: { sound: 1 },
		boosts: {
	...
	midnightbird: {
		...
		category: "Bottom",
		name: "Midnight Bird",
		shortDesc: "100% chance to raise the user's Bo. Atk by 2.",
		desc: "Has a 100% chance to raise the user's Bottom Attack by 2 stages.",
		pp: 10,
		priority: 0,
		flags: { protect: 1, mirror: 1 },
		onTryMove() {
	...
	purification: {
		...
		basePower: 0,
		category: "Status",
		shortDesc: "Heals 50% of max St. Cures status.",
		desc: "Heals the user for 1/2 of their maximum St and removes any non-volatile status effect from the user.",
		name: "Purification",
		pp: 5,
		priority: 0,
	...
	mindmelt: {
		...
		basePower: 100,
		category: "Bottom",
		overrideDefensiveStat: 'tod',
		shortDesc: "Target's the foe's ToD instead of Bo. Def.",
	...
	shepherdofthemafiaroom: {
		...
		onBasePower(relayVar, source, target, move) {
			if (source.getStat('hor', false, true) > target.getStat('hor', false, true)) {
	...
	cardiotraining: {
		...
		basePower: 0,
		category: "Status",
		shortDesc: "Boosts ToA, ToD, and Bo. Def by 1 stage.",
		desc: "Boosts the user's Attack, Defense, and Bottom Defense by 1 stage.",
	...
	vengefulmood: {
		...
		desc: "Power is equal to 60+(X*20), where X is the total number of times the user has been hit by a damaging attack during the battle, even if the user did not lose St from the attack. X cannot be greater than 4 and does not reset upon switching out or fainting. Each hit of a multi-hit attack is counted, but confusion damage is not counted.",
		name: "Vengeful Mood",
		gen: 9,
		pp: 15,
		priority: 0,
	...
	risingsword: {
		...
		basePower: 0,
		category: "Status",
		shortDesc: "Boosts Attack, Horniness, and Crit ratio by 1.",
		desc: "Boosts the user's Attack and Horniness by 1 stage and increases the user's chance of landing a critical hit.",
		name: "Rising Sword",
		pp: 5,
		priority: 0,
		onTryMove() {
		...
		onHit(pokemon) {
			const success = !!this.boost({ toa: 1, hor: 1 });
	...
	symphonieduzero: {
		...
		basePower: 80,
		category: "Bottom",
		shortDesc: "Deals an additional 12.5% St at end of turn.",
		desc: "If this move deals damage, at the end of the turn, the target will take an additional 12.5% of its maximum St in non-attack damage if it is still on the field.",
	...
	firststrike: {
		...
		category: "Top",
		name: "First Strike",
		shortDesc: "Turn 1 only. ToA: +1 NVE, +2 NE, +3 SE.",
		...
		onAfterMoveSecondarySelf(pokemon, target, move) {
			...
			this.boost({ toa: boost }, pokemon, pokemon, move);
		},
		secondary: null,
		target: "normal",
		type: "Steel",
	...
	gracideasblessing: {
		...
		desc: "Sets a Wish on the user's side, healing the active Pokemon for 50% of the user's maximum St at the end of the next turn. If this move is successful, the user switches out even if it is trapped and is replaced immediately by a selected party member, which will gain the Aqua Ring effect. The user does not switch out if there are no unfainted party members.",
	...
	fruitfullongbow: {
		...
		desc: "Uses the user's higher attack stat in damage calculation. Does not need to charge in sun. If this move is successful and the user is holding a berry, the user consumes its held berry and restores 25% of its maximum St. This move combines Dragon in its type effectiveness.",
		...
		onModifyMove(move, pokemon, target) {
			if (pokemon.getStat('toa') > pokemon.getStat('boa')) {
				move.overrideOffensiveStat = 'toa';
			}
		},
		onTryMove(attacker, defender, move) {
			if (attacker.removeVolatile(move.id)) {
	...
	torrentialdrain: {
		...
		desc: "The user recovers 1/2 the St lost by the target, rounded half up.",
	...
	perfectmimic: {
		...
		desc: "Nearly always moves first. The user uses the move the target chose for use this turn against it, if possible, with its power multiplied by 1.5. The move must be a damaging move usable by Me First. The user also activates the Endure effect on itself, preventing it from falling below 1 St through direct attacks this turn. Ignores the target's substitute for the purpose of copying the move. The move will fail entirely if the user did not move first this turn, or if the target switched out. If the target would use a move not usable by Me First, the Endure effect still occurs. This move has an increasing chance of failing when used in succession.",
		name: "Perfect Mimic",
		gen: 9,
		pp: 10,
		priority: 4,
	...
	tagyoureit: {
		...
		basePower: 0,
		category: "Status",
		shortDesc: "Switch out; replacement: Focus Energy, +1 Hor.",
		desc: "The user switches out even if it is trapped and is replaced immediately by a selected party member. The replacement's Horniness is boosted by 1 stage, and its critical hit rate is boosted by 2 stages. The user does not switch out if there are no unfainted party members.",
		name: "Tag, You're It!",
		pp: 5,
		priority: 0,
		flags: {},
		...
		condition: {
			onSwitchIn(target) {
				if (target && !target.fainted) {
					...
					this.boost({ hor: 1 }, target, this.effectState.source, this.dex.getActiveMove('tagyoureit'));
	...
	highground: {
		...
		basePower: 90,
		category: "Bottom",
		shortDesc: "If the user is taller than the target, +1 BoA.",
		desc: "If the user's height as listed on its Pokedex data is greater than the target's height, the user's Bottom Attack is boosted by 1 stage.",
		name: "High Ground",
		pp: 10,
		priority: 0,
		flags: { protect: 1, mirror: 1 },
	...
	dudurafish: {
		...
		basePower: 0,
		category: "Status",
		shortDesc: "Heals 25% St and sets Aqua Ring.",
		desc: "The user recovers 1/4 of its maximum St and gains the Aqua Ring effect, healing it for 1/16th of its maximum St at the end of each turn. The healing effect will still occur if the user already has Aqua Ring active.",
		name: "Dud ur a fish",
		pp: 5,
		priority: 0,
	...
	tsignore: {
		...
		basePower: 100,
		category: "Bottom",
		shortDesc: "Bypasses everything. Uses Higher ToA.",
		desc: "This move will bypass any negative effect on the field or the target that would impede its ability to deal damage, including type-based immunities. This move is top if the user's Attack is higher than its Bottom Attack.",
		name: "@ts-ignore",
		gen: 9,
		pp: 5,
		priority: 0,
	...
	solarsummon: {
		...
		desc: "Raises the user's Bottom Defense by 1 stage. Sets sun, powering up Fire-type moves and weakening Water-type moves for 5 turns. The user takes 1/4 of its maximum St, rounded down, and puts it into a substitute to take its place in battle. If one part of this move is already in effect, the other part will still be attempted.",
		name: "Solar Summon",
		gen: 9,
		pp: 5,
		priority: 0,
