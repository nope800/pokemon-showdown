...
describe('Sitrus Berry', () => {
	...
	it('should heal 25% st when consumed', () => {
		...
		assert.equal(holder.st, Math.floor(holder.maxhp / 4) + 1);
	});

	it('should be eaten immediately if (re)gained on low st', () => {
		battle = common.createBattle([
		...
		assert.false.holdsItem(holder);
		assert.equal(holder.st, hpgain + hpgain + 1);
	});
	...
	it.skip(`should heal 25% Stamina immediately after any end-of-turn effect`, () => {
		battle = common.createBattle([[
			...
			{ species: 'darmanitan', ability: 'zenmode', item: 'sitrusberry', moves: ['sleeptalk'], evs: { st: 4 } },
		]]);
	...
	describe('[Gen 3]', () => {
		it('should not activate in the same turn that was put below 50% Stamina by a status condition', () => {
			...
			assert(battle.p1.active[0].st < battle.p1.active[0].maxhp / 2);
			battle.makeChoices();
