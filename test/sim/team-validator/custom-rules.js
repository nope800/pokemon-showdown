...
describe("Custom Rules", () => {
	it('should support legality tags', () => {
		let team = [
			{ species: 'kitsunoh', ability: 'frisk', moves: ['shadowstrike'], evs: { st: 1 } },
		];
		assert.false.legalTeam(team, 'gen7anythinggoes');
		assert.legalTeam(team, 'gen7anythinggoes@@@+cap');

		team = [
			{ species: 'pikachu', ability: 'airlock', moves: ['thunderbolt'], evs: { st: 1 } },
		];
		assert.false.legalTeam(team, 'gen7anythinggoes');
		assert.legalTeam(team, 'gen7ou@@@!obtainableabilities');

		team = [
			{ species: 'pikachu', ability: 'airlock', moves: ['dragondance'], evs: { st: 1 } },
		];
	...
	it('should allow Pokemon to be banned', () => {
		...
		team = [
			{ species: 'greninjaash', ability: 'battlebond', moves: ['surf'], evs: { st: 1 } },
		];
		assert.false.legalTeam(team, 'gen7anythinggoes@@@-Greninja-Bond');

		team = [
			{ species: 'greninjabond', ability: 'battlebond', moves: ['surf'], evs: { st: 1 } },
		];
	...
	it('should allow Pokemon to be unbanned', () => {
		const team = [
			{ species: 'blaziken', ability: 'blaze', moves: ['skyuppercut'], evs: { st: 1 } },
		];
	...
	it('should allow Pokemon to be whitelisted', () => {
		let team = [
			{ species: 'giratina', ability: 'pressure', moves: ['protect'], evs: { st: 1 } },
		];
		assert.legalTeam(team, 'gen7ubers@@@-allpokemon,+giratinaaltered');

		team = [
			{ species: 'giratinaorigin', ability: 'levitate', moves: ['protect'], evs: { st: 1 } },
		];
		assert.false.legalTeam(team, 'gen7ubers@@@-allpokemon,+giratinaaltered');

		// -allpokemon should override +past
		team = [
			{ species: 'tyrantrum', ability: 'strongjaw', moves: ['protect'], evs: { st: 1 } },
		];
		assert.false.legalTeam(team, 'gen8nationaldex@@@-allpokemon');

		// +pikachu should not override -past
		team = [
			{ species: 'pikachu-belle', ability: 'lightningrod', moves: ['thunderbolt'], evs: { st: 1 } },
		];
	...
	it('should allow Pokemon to be force-whitelisted', () => {
		...
		let team = [
			{ species: 'cloyster', ability: 'skilllink', moves: ['iciclespear'], evs: { st: 1 } },
		];
		...
		team = [
			{ species: 'pikachu', ability: 'lightningrod', moves: ['thunderbolt'], evs: { st: 1 } },
		];
		assert.legalTeam(team, 'gen9ou@@@-allpokemon,+pikachu');
		assert.throws(() => Dex.formats.validate('gen9ou@@@+pikachu,-allpokemon'));

		team = [
			{ species: 'zygarde-10', ability: 'aurabreak', moves: ['bodyslam'], evs: { st: 1 } },
		];
	...
	it('should support banning/unbanning tag combinations', () => {
		let team = [
			{ species: 'Crucibelle-Mega', ability: 'Regenerator', moves: ['protect'], evs: { st: 1 } },
		];
		...
		team = [
			{ species: 'Moltres-Galar', ability: 'Berserk', moves: ['protect'], evs: { st: 1 } },
		];
	...
	it('should support restrictions', () => {
		let team = [
			{ species: 'Yveltal', ability: 'No Ability', moves: ['protect'], evs: { st: 1 } },
		];
		assert.legalTeam(team, 'gen7customgame@@@limitonerestricted,*restrictedlegendary');

		team = [
			{ species: 'Yveltal', ability: 'No Ability', moves: ['protect'], evs: { st: 1 } },
			{ species: 'Xerneas', ability: 'No Ability', moves: ['protect'], evs: { st: 1 } },
		];
	...
	it('should allow moves to be unbanned', () => {
		const team = [
			{ species: 'absol', ability: 'pressure', moves: ['batonpass'], evs: { st: 1 } },
		];
	...
	it('should allow items to be banned', () => {
		let team = [
			{ species: 'pikachu', ability: 'static', moves: ['agility', 'protect', 'thunder', 'thunderbolt'], item: 'lightball', evs: { st: 1 } },
		];
	...
	it('should allow items to be unbanned', () => {
		const team = [
			{ species: 'eevee', ability: 'runaway', moves: ['tackle'], item: 'eeviumz', evs: { st: 1 } },
		];
	...
	it('should allow abilities to be unbanned', () => {
		const team = [
			{ species: 'wobbuffet', ability: 'shadowtag', moves: ['counter'], evs: { st: 1 } },
		];
	...
	it('should allow complex bans to be added', () => {
		...
		team = [
			{ species: 'smeargle', ability: 'owntempo', moves: ['gravity'], evs: { st: 1 } },
			{ species: 'pikachu', ability: 'static', moves: ['thunderbolt'], evs: { st: 1 } },
		];
	...
	it('should allow complex bans to be altered', () => {
		let team = [
			{ species: 'smeargle', ability: 'owntempo', moves: ['gravity'], evs: { st: 1 } },
			{ species: 'abomasnow', ability: 'snowwarning', moves: ['grasswhistle'], evs: { st: 1 } },
		];
		...
		team = [
			{ species: 'smeargle', ability: 'owntempo', moves: ['gravity'], evs: { st: 1 } },
			{ species: 'abomasnow', ability: 'snowwarning', moves: ['grasswhistle'], evs: { st: 1 } },
			{ species: 'cacturne', ability: 'sandveil', moves: ['grasswhistle'], evs: { st: 1 } },
		];
	...
	it('should allow rule bundles to be removed', () => {
		const team = [
			{ species: 'azumarill', ability: 'hugepower', moves: ['waterfall'], evs: { st: 1 } },
			{ species: 'azumarill', ability: 'hugepower', moves: ['waterfall'], evs: { st: 1 } },
		];
	...
	it('should allow rule bundles to be overridden', () => {
		const team = [
			{ species: 'charizard-mega-y', ability: 'drought', item: 'charizarditey', moves: ['wingattack'], evs: { st: 1 } },
		];
