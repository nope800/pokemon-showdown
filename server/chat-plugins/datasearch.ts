...
export const commands: Chat.ChatCommands = {
	...
	dexsearchhelp() {
		this.sendReplyBox(
			...
			`Inequality ranges use the characters <code>>=</code> for <code>≥</code> and <code><=</code> for <code>≤</code>; e.g., <code>st <= 95</code> searches all Pok\u00e9mon with St less than or equal to 95; <code>tier <= uu</code> searches all Pok\u00e9mon in singles tiers lower than UU.<br/>` +
...
function runItemsearch(target: string, cmd: string, message: string) {
	...
	for (const [i, search] of rawSearch.entries()) {
		...
		switch (newWord) {
		// Words that don't really help identify item removed to horniness up search
		case 'a':
		case 'an':
		case 'is':
		case 'it':
